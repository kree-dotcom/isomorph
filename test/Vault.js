...
describe("Vault contract", function () {
  ...
  before(async function () {
        ...
        //placeholder, use sUSD properties for lyra collateral right now
        await collateralBook.addCollateralType(lyraLPToken.address, lyraCode, sUSDMinMargin, sUSDLiqMargin, sUSDInterest, LYRA, lyraLiqPool.address);
  ...
  describe("OpenLoans", function () {
    ...
    it("Should mint user moUSD if a loan is opened with Lyra LP tokens", async function () {
      this.timeout(100000);
      const beforeAddr1Balance = await moUSD.balanceOf(addr1.address);
    ...
    it("Should function for non-Lyra collateral during Lyra Circuit Breaker triggered", async function () {
      const beforeAddr1Balance = await moUSD.balanceOf(addr1.address);
      expect(beforeAddr1Balance).to.equal(0);
      const beforeTreasuryBalance = await moUSD.balanceOf(treasury.address);
      expect(beforeTreasuryBalance).to.equal(0);
      ...
      await network.provider.send("hardhat_setStorageAt", [
        ...
        "0x21",
        "0x0000000000000000000000000000000000000000000000000000000075A07897", //1973450903 epoch time i.e. 2032 july
      ]);
      await expect(vault.connect(addr1).openLoan(sUSD.address, collateralUsed, loanTaken)).to.emit(vault, 'OpenOrIncreaseLoan').withArgs(addr1.address, loanTaken, sUSDCode,collateralUsed );
      const AfterAddr1Balance = await moUSD.balanceOf(addr1.address);
      expect(AfterAddr1Balance).to.equal(beforeAddr1Balance.add((loanTaken.mul(base.sub(loanOpenfee))).div(base)))
    ...
    it("Should openLoan and record debt corrected after time elasped in system", async function () {   
      ...
      let virtualPriceDelta = await collateralBook.viewVirtualPriceforAsset(sUSD.address);
      const AfterAddr1Balance = await moUSD.balanceOf(addr1.address);
      expect(AfterAddr1Balance).to.equal(beforeAddr1Balance.add((loanTaken.mul(base.sub(loanOpenfee))).div(base)))
      const AfterTreasuryBalance = await moUSD.balanceOf(treasury.address);
      expect(AfterTreasuryBalance).to.equal(loanTaken.mul(loanOpenfee).div(base));
      const virtualDebtBalance = await vault.moUSDLoaned(sUSDaddr, addr1.address);
     ...
     it("Should correctly apply interest accrued after a long time", async function () {
      ...
      const virtualDebtBalance = await vault.moUSDLoaned(sUSD.address, addr1.address);
      let virtualPriceUpdate = await collateralBook.viewVirtualPriceforAsset(sUSD.address); 
    ...
    it("Should be possible to increase existing loan and emit OpenLoan event", async function () {
      await sUSD.connect(addr1).approve(vault.address, collateralUsed);
  ...
  describe("increaseCollateralAmount", function () {
    ...
    it("Should function for non-Lyra collateral during Lyra Circuit Breaker triggered", async function () {
      ...
      const beforeAddr1Collateral = await sUSD.balanceOf(addr1.address);
      const collateralAdded = totalCollateralUsing.sub(collateralUsed)
      await sUSD.connect(addr1).approve(vault.address, collateralAdded);
      //trigger Lyra CB here by overwriting CBTimestamp in slot 33 of LiquidityPool storage
      await network.provider.send("hardhat_setStorageAt", [
        "0x5Db73886c4730dBF3C562ebf8044E19E8C93843e",
        "0x21",
    ...
    it("Should fail if accrued interest means debt is still too large", async function () {
      ...
      await collateralBook.queueCollateralChange(sUSD.address, sUSDCode, sUSDMinMargin, sUSDLiqMargin, sUSDInterest, ZERO_ADDRESS);
    ...
    it("Should fail if sender never opened loan originally ", async function () {
      ...
      await expect(vault.connect(addr1).increaseCollateralAmount(lyraLPToken.address, collateralAdded)).to.be.revertedWith("No existing collateral!");
    });
  ...
  describe("CloseLoans", function () {
    ...
    beforeEach(async function () {
      ...
      await sUSD.connect(addr1).approve(vault.address, collateralAmount);
      await vault.connect(addr1).openLoan(sUSDaddr, collateralAmount, loanAmount);
      ...
      await vault.connect(addr2).openLoan(sUSDaddr, collateralAmount, loanAmount);
    ...
    it("Should return user moUSD if valid conditions are met and emit ClosedLoan event", async function () {
      ...
      let realDebt = await vault.moUSDLoaned(sUSD.address, addr1.address);
      let virtualPrice = await collateralBook.viewVirtualPriceforAsset(sUSD.address);
    ...
    it("Should allow reducing margin ratio if in excess by drawing out collateral", async function () {
      ...
      const valueClosing = realDebt.mul(virtualPrice).div(e18);
      const beforeMoUSDBalance = await moUSD.balanceOf(addr1.address);
      //expect(beforeMoUSDBalance).to.equal(valueClosing);
      const beforeColBalance = await sUSD.balanceOf(addr1.address);
      const requestedCollateral = collateralAmount
      await moUSD.connect(addr1).approve(vault.address, valueClosing);
      await expect(vault.connect(addr1).closeLoan(sUSDaddr, requestedCollateral, valueClosing)).to.emit(vault, 'ClosedLoan').withArgs(addr1.address, valueClosing, sUSDCode, requestedCollateral);
    ...
    it("Should function for non-lyra collateral during Lyra Circuit Breaker triggered", async function () {
      ...
      const beforeMoUSDBalance = await moUSD.balanceOf(addr1.address);
      const beforeColBalance = await sUSD.balanceOf(addr1.address);
      const requestedCollateral = ethers.utils.parseEther("1000");
      await moUSD.connect(addr1).approve(vault.address, valueClosing);
      //trigger Lyra CB here by overwriting CBTimestamp in slot 33 of LiquidityPool storage
      await network.provider.send("hardhat_setStorageAt", [
        "0x5Db73886c4730dBF3C562ebf8044E19E8C93843e",
    ...
    it("Should function for Lyra collateral only once Lyra Circuit Breaker time passes", async function () {
      ...
      let realDebt = await vault.moUSDLoaned(lyraLPToken.address, addr1.address);
    ...
    it("Should fail to close if the market is closed", async function () {
      await suspend_synth(provider, sUSDCode);
      await expect(
        vault.connect(addr1).closeLoan(sUSDaddr, collateralAmount, loanAmount)
      ).to.be.revertedWith("Synth is suspended. Operation prohibited");
    });
    ...
    it("Should fail to close if user asks for more collateral than originally posted", async function () {
      await expect(
        vault.connect(addr1).closeLoan(sUSDaddr, collateralAmount.add(1), loanAmount)
      ).to.be.revertedWith("User never posted this much collateral!");

    });

    it("Should fail to close if user has insufficient MoUSD", async function () {
      const moUSDAmount = await moUSD.balanceOf(addr1.address);
    ...
    it("Should fail to close if user tries to return more MoUSD than borrowed originally", async function () {
      ...
      await expect(
        ...
        vault.connect(addr1).closeLoan(sUSDaddr, collateralAmount, loanAmount.mul(11).div(10))
    ...
    it("Should fail to close if partial loan closure results in an undercollateralized loan", async function () {
      await sUSD.connect(owner).transfer(addr2.address, collateralAmount);
      await sUSD.connect(addr2).approve(vault.address, collateralAmount);
      await vault.connect(addr2).openLoan(sUSDaddr, collateralAmount, collateralAmount.div(2));

      //attempt to take back all collateral repaying nothing
      await expect(
        vault.connect(addr2).closeLoan(sUSDaddr, collateralAmount, 0)
      ).to.be.revertedWith("Remaining debt fails to meet minimum margin!");
      //attempt to take back all collateral repaying some of loan
      await expect(
        vault.connect(addr2).closeLoan(sUSDaddr, collateralAmount, collateralAmount.div(3))
      ).to.be.revertedWith("Remaining debt fails to meet minimum margin!");
    });
  ...
  describe("callLiquidation", function () {
    ...
    beforeEach(async function () {
      ...
      await collateralBook.TESTchangeCollateralType(sETH.address, sETHCode, sETHMinMargin2, sETHLiqMargin2, sETHInterest2, ZERO_ADDRESS, liq_return.mul(2));   //fake LIQ_RETURN used for ease of tests   
      ...
      await collateralBook.TESTchangeCollateralType(sETH.address, sETHCode, sETHMinMargin3, sETHLiqMargin3, sETHInterest2, ZERO_ADDRESS, liq_return.mul(2)); //fake LIQ_RETURN used for ease of tests
      ...
      let debt = (await vault.moUSDLoaned(sETHaddr, addr1.address)).mul(openingVirtualPrice).div(e18)
      //debt = Math.ceil(debt);
    ...
    it("Should liquidate if entire loan is eligible to liquidate and emit Liquidation & BadDebtCleared events", async function () {
      ...
      await collateralBook.TESTchangeCollateralType(sETH.address, sETHCode, sETHMinMargin4, sETHLiqMargin4, sETHInterest4, ZERO_ADDRESS, liq_return);      
      ...
      const virtualDebtBegin = await vault.moUSDLoaned(sETHaddr, addr1.address);
      ethPriceBN =  await vault.priceCollateralToUSD(sETHCode, e18, SYNTH);
      await vault.viewLiquidatableAmount(colQuantity, ethPriceBN, loanSizeInMoUSD, sETHLiqMargin4)
      ...
      const realDebt = (await vault.moUSDLoaned(sETHaddr, addr1.address)).mul(virtualPriceEnd).div(e18)
    ...
    it("Should partially liquidate loan if possible and emit Liquidation event", async function () {
      ...
      await collateralBook.TESTchangeCollateralType(sETH.address, sETHCode, sETHMinMargin, sETHLiqMargin, sETHInterest5, ZERO_ADDRESS, liq_return.mul(2)); //fake LIQ_RETURN used for ease of tests
      ...
      await collateralBook.TESTchangeCollateralType(sETH.address, sETHCode, sETHMinMargin5, sETHLiqMargin5, sETHInterest5, ZERO_ADDRESS, liq_return.mul(2)); //fake LIQ_RETURN used for ease of tests
      const virtualDebtBegin = await vault.moUSDLoaned(sETHaddr, addr1.address);
    ...
    it("Should liquidate for a non-Lyra collateral if Lyra circuit breaker is triggered", async function () {
      liq_return = await vault.LIQUIDATION_RETURN();
      const helperAmount = ethers.utils.parseEther("500");
      const helperLoan = helperAmount.div(2);
      await sUSD.connect(owner).transfer(addr2.address, helperAmount);
      await sUSD.connect(addr2).approve(vault.address, helperAmount);
      const beforeLoanMoUSD = await moUSD.balanceOf(addr2.address);
      ...
      const sETHInterest4 = ethers.utils.parseEther("1.19710969"); //1.001^180 i.e. 3 mins continiously compounding per second
      await collateralBook.TESTchangeCollateralType(sETH.address, sETHCode, sETHMinMargin4, sETHLiqMargin4, sETHInterest4, ZERO_ADDRESS, liq_return); 
      const totalCollateralinMOUSD =  await vault.priceCollateralToUSD(sETHCode, colQuantity, SYNTH);
      const amountLiquidated = totalCollateralinMOUSD.mul(base.sub(liquidatorFeeBN)).div(base); 
      const virtualDebtBegin = await vault.moUSDLoaned(sETHaddr, addr1.address);
      ethPriceBN =  await vault.priceCollateralToUSD(sETHCode, e18, SYNTH);
      await vault.viewLiquidatableAmount(colQuantity, ethPriceBN, loanSizeInMoUSD, sETHLiqMargin4)
    ...
    it("Should liquidate for a Lyra collateral with triggered Circuit Breaker only once its time has expired", async function () {
      ...
      await collateralBook.TESTchangeCollateralType(lyraLPToken.address, lyraCode, MinMargin, LiqMargin, Interest, lyraLiqPool.address, liq_return.mul(2));   //fake LIQ_RETURN used for ease of tests   
      ...
      expect(beforeColBalance).to.equal(collateralUsed);
      ...
      tx = await collateralBook.TESTchangeCollateralType(lyraLPToken.address, lyraCode, MinMargin, LiqMargin, Interest, lyraLiqPool.address, liq_return.mul(2));   //fake LIQ_RETURN used for ease of tests   
    ...
    it("Should revert if liquidator lacks moUSD to repay debt", async function () {
      ...
      const sETHInterest4 = ethers.utils.parseEther("1.19710969"); //1.001^180 i.e. 3 mins continiously compounding per second
      await collateralBook.TESTchangeCollateralType(sETH.address, sETHCode, sETHMinMargin4, sETHLiqMargin4, sETHInterest4, ZERO_ADDRESS, liq_return); 
      await expect(vault.connect(addr2).callLiquidation(addr1.address, sETHaddr)).to.be.revertedWith('ERC20: burn amount exceeds balance');
    });
